...
class SendEmail:
    ...
    path = os.path.join(rootPath, r"test_report\result.html")  # 报告所在路径

    def __init__(self, defultPath = path):
        emailDIC = ReadYaml().readByKey('email')  # yaml配置文件中读取email相关地址及邮箱授权码
    ...
    def sendFJ(self):
        ...
        '''
        mail = {
            'subject': '接口自动化测试报告'+time.strftime("%Y-%m-%d %X"),  # Anything you want.
